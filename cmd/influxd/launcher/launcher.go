...
func (m *Launcher) run(ctx context.Context) (err error) {
	...
	var orgHTTPServer *tenant.OrgHandler
	{
		...
		urmHandler := tenant.NewURMHandler(m.log.With(zap.String("handler", "urm")), platform.OrgsResourceType, "id", userSvc, tenant.NewAuthedURMService(orgSvc, tenant.NewAuthedURMService(orgSvc, userResourceSvc)))
